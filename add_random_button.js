...
const jumpToRandomPost = async() => {
    ...
    const randomPageN = randomPage(NumPages);
...
// current page number
var parseCurrentPage = () => parseInt(document.querySelector('button[aria-current], li a[aria-current]').textContent)

var NavBars = ['nav.pager ul','div+nav']
var NavbarElm = selector => {
    return new Promise(res => {
        var intrv = setInterval(() => {
            var targetSelector = NavBars.find(selector => document.querySelector(selector));
...
var addRandomButton = async() => {

    var {ELEMENT,IS_MOBILE} = await NavbarElm();
    console.log("IS_MOBILE : ",IS_MOBILE);
    console.log("ELEMENT   : ",ELEMENT);

    var RandomHREF = document.createElement("button");
